# taylor.rb written by Nehemiah Andrews
# This program was written in response to the 
# {Lesson 27 Coding Example} from theFirehoseProject

require_relative 'banner'

print "Do you like Taylor Swift (enter Yes or No)? "
value = gets.chop.capitalize

if value != 'Yes'
  puts "And the haters gonna hate, hate, hate, hate, hate"
  puts "Baby, I'm just gonna shake, shake, shake, shake, shake"
  puts "I shake it off, I shake it off"
end

puts "Uh oh!  I don't know about you! But I'm feeling 22"